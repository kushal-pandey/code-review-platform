...
export default function SnippetView() {
  ...
  const handleSendComment = () => {
    if (!newComment.trim() || !currentUser || !stompClientRef.current?.connected) return
  ...
  if (!snippet) {
    return (
      <div style={{ color: '#e6edf3', textAlign: 'center', marginTop: 100, background: '#0d1117', height: '100vh' }}>
  ...
  return (
    <div style={{ background: '#0d1117', height: '100vh', display: 'flex', flexDirection: 'column', color: '#e6edf3' }}>
      {/* Navbar */}
      <nav style={{
        background: '#161b22', padding: '0 24px', height: '60px',
        display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        borderBottom: '1px solid #30363d', flexShrink: 0
      }}>
        <h2 style={{ margin: 0, color: '#58a6ff', cursor: 'pointer', fontSize: '1.1rem' }} onClick={() => navigate('/')}>
          ← CodeReview
        </h2>
        <div style={{ textAlign: 'center' }}>
          <span style={{ color: '#e6edf3', fontWeight: 600 }}>{snippet.title}</span>
          <span style={{ color: '#8b949e', fontSize: '0.8rem', display: 'block' }}>
            by {snippet.author?.username}
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{
            background: '#1f6feb', padding: '4px 12px', borderRadius: '20px',
          ...
          <span style={{
            ...
            background: connected ? '#2ea043' : '#f85149', display: 'inline-block'
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <Editor
            ...
            theme="vs-dark"
            options={{
              fontSize: 14, readOnly: true, minimap: { enabled: true },
        ...
        <div style={{
          ...
          background: '#161b22', borderLeft: '1px solid #30363d', flexShrink: 0
        }}>
          {/* Panel Header */}
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #30363d', flexShrink: 0 }}>
            <h4 style={{ margin: 0, color: '#e6edf3' }}>
              💬 Discussion
              <span style={{
                marginLeft: 8, background: '#30363d', color: '#8b949e',
                padding: '2px 8px', borderRadius: '10px', fontSize: '0.75rem', fontWeight: 400
              }}>
                {comments.length}
              </span>
            </h4>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px' }}>
            {comments.length === 0 && (
              <div style={{ textAlign: 'center', padding: '40px 20px', color: '#8b949e' }}>
                ...
                <p style={{ fontSize: '0.875rem' }}>No comments yet.<br />Start the code review!</p>
              </div>
            )}

            {comments.map(comment => (
              <div key={comment.id} style={{
                background: '#0d1117', borderRadius: '8px', padding: '12px 14px',
                marginBottom: '10px', border: '1px solid #30363d'
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {comment.lineNumber && (
                      <span style={{
                        marginLeft: 8, background: '#1f6feb22', color: '#58a6ff',
                ...
                <p style={{ margin: 0, fontSize: '0.875rem', color: '#c9d1d9', lineHeight: 1.5, wordBreak: 'break-word' }}>
                  {comment.content}
                </p>
              </div>
            ))}
          ...
          {/* Comment Input */}
          <div style={{ padding: '14px', borderTop: '1px solid #30363d', flexShrink: 0 }}>
            <input
              type="number"
              value={lineNumber}
              ...
              style={{
                ...
                marginBottom: '8px', fontSize: '0.875rem', outline: 'none',
            ...
            <textarea
              ...
              style={{
                width: '100%', background: '#0d1117', border: '1px solid #30363d',
                borderRadius: '6px', padding: '10px 12px', color: '#e6edf3',
                resize: 'none', marginBottom: '8px', fontSize: '0.875rem',
            ...
            <button
              ...
              style={{
                ...
                border: 'none', cursor: newComment.trim() && connected ? 'pointer' : 'not-allowed',
                background: newComment.trim() && connected ? '#238636' : '#21262d',
